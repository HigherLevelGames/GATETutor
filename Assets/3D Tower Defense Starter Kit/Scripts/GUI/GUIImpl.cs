...
public class GUIImpl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if ((Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || !gui.mobile && Input.GetMouseButtonUp(0))
            ...
            && SV.selection && gui.currentGrid != SV.gridSelection))
        {
            ...
            if ((gui.mobile && EventSystem.current.IsPointerOverGameObject(Input.GetTouch(0).fingerId))
        ...
        switch (buildMode)
        {
            ...
            case BuildMode.Grid:
            {
                ...
                if ((Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || 
                     !gui.mobile && Input.GetMouseButtonUp(0)) && gui.CheckIfGridIsFree() && !SV.selection)
                {
    ...
    void ProcessPowerUps()
    {
        ...
        if (gui.IsPowerUpSelected())
        {
            ...
            {
                //don't reposition powerup indicator directly on the button on mobiles
                if (!gui.mobile || Input.touchCount > 0 && Input.GetTouch(0).phase != TouchPhase.Ended && Input.GetTouch(0).phase != TouchPhase.Began
                    && !EventSystem.current.IsPointerOverGameObject(Input.GetTouch(0).fingerId))
                    SV.powerUpIndicator.transform.position = gui.GetPowerUp().position + new Vector3(0, 0.25f, 0);
            }

            //try to execute powerup on mouse click (up on mobiles)
            if (!gui.mobile && Input.GetMouseButtonUp(0) ||
                Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended &&
                !EventSystem.current.IsPointerOverGameObject(Input.GetTouch(0).fingerId) &&
                EventSystem.current.lastSelectedGameObject != null)
            {
                gui.ActivatePowerUp();
            }
        }
    ...
    public class con
    {
        ...
        public Image relSprite;    //foreground bar sprite for changing its color
    }
}